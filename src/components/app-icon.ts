...
class AppIcon extends HTMLElement {
  ...
  connectedCallback() {
    const shape = <string>this.shape;
    const template = <HTMLTemplateElement>document.getElementById("template-icon");
    const fragment = <DocumentFragment>template.content.cloneNode(true);
    const svg = <SVGElement>fragment.querySelector("svg");
    svg.setAttribute("viewBox", icon[shape].viewBox);
    svg.innerHTML = icon[shape].path;
    if (this.iconWidth) svg.style.width = this.iconWidth;
    if (this.iconHeight) svg.style.height = this.iconHeight;
